@isset($product)
    <a href="{{ route('product', ['id' => $product->id]) }}" id='product-{{ $product->id }}'
        @if (!empty($product->images) && count($product->images) > 0) style="background-image: url('{{ URL::asset($product->images[0]->url) }}'); background-position: center;  {{ isset($suggestion) ? 'background-size: cover;' : 'background-size: cover;' }}" @endif
        ...
        <div class="card-data flex flex-col items-start justify-between h-full">
            ...
            <div class="card__title-container w-full flex justify-between items-center z-10">
                ...
                <div data-product-id="{{ $product->id }}" onclick="addToCart(event, {{ $product->id }})"
    ...
    @push('scripts')
        ...
        <script>
            function addToCart(event, productId) {
                ...
                axios.post(
                        "@auth {{ route('cart.store') }} @endauth @guest {{ route('cart.store.guest') }} @endguest", {
